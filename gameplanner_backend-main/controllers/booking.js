  ...
  try {
    ...
    if(equipmentCount == 0) {
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: process.env.BASE_URL + "/BookingSuccess",
        cancel_url: process.env.BASE_URL + "/BookingFailure",
      });
    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        if(metadata.type === 'Booking') {
          ...
          await axios
            .post(process.env.URL + "/api/createBooking", {
          ...
          await axios
          .post(process.env.URL + "/api/membershipUpdate", {
