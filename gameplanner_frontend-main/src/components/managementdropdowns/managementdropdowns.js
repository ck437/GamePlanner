...
export default function ManagementDropdowns(props) {
  ...
  const fetchfranchiseData = async () => {
    try {
      ...
      const response = await axios.put(process.env.URL + '/api/dropdown/franchise',data, { headers: {
    ...
    const fetchstateData = async ()=>{
        try {
          ...
          const response = await axios.put(process.env.URL + '/api/dropdown/state',statedata, { headers: {
            'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
            'Content-Type': 'application/json'
          }});
          if (response.data.success) {
    ...
    const fetchcityData = async ()=>{
        try {
          ...
          const response = await axios.put(process.env.URL + '/api/dropdown/city',citydata, { headers: {
            'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
            'Content-Type': 'application/json'
          }});
          if (response.data.success) {
    ...
    const fetchcompanyData = async ()=>{
      try {
        ...
        const response = await axios.put(process.env.URL + '/api/dropdown/company',companydata, { headers: {
          'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
          'Content-Type': 'application/json'
        }});
        if (response.data.success) {
