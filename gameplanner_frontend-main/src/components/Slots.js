...
const fetchroomsdata = async() => {
  try {
    ...
    const response = await axios.get(process.env.URL + `/api/rooms/${BookingData.activity}`,{ headers: {
...
const fetchslotsdata = async() => {
  try {
    ...
    const response = await axios.post(process.env.URL + `/api/slots`,params,{ headers: {
      'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
      'Content-Type': 'application/json'
    }});
    if (response.data.success) {
