...
export default function Summary() {
  ...
  const fetchmemberdata = async() => {
    try{
      const userResponse = await axios.get(process.env.URL + `/api/user/${BookingData.email}`, { headers: {
  ...
  const fetchsummarydata = async() => {
    try {
      ...
      const response = await axios.post(process.env.URL + `/api/price`,params,{ headers: {
        'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
      }});
  ...
  const handleClickProceed = async () => {
    ...
    try {
      const response = await axios.post(process.env.URL + '/api/createBooking', BookingData, { headers: {
      'Authorization': 'Bearer '+cookies.get("jwt_authorization")
    }});
      if(response.data.success){
  ...
  const handleClickProceedandCheckout = async() => {
    const input = {'booking': BookingData,
                    'price': summaryData};
    try {
      const response = await axios.post(process.env.URL + `/api/checkout`,input,{ headers: {
        'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
      }});
