...
export default function Membership() {
  ...
  const fetchdata = async() => {
    try {
        const response = await axios.get(process.env.URL + `/api/user/${user.email}`, { headers: {
  ...
  const handleClickProceedandCheckout = async() => {
    ...
    const response = await axios.post(process.env.URL + `/api/membershipCheckout`,input,{ headers: {
        'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
      }});
