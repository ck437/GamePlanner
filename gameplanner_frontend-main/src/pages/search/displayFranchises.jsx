...
function DisplayFranchises() {
  ...
  const fetchFranchises = async () => {
    try {
      if (sortBy === "distance") {
        const response = await axios.put(process.env.URL + `/api/franchisesByDistance`, param, { headers: {
        ...
        const response = await axios.put(process.env.URL + `/api/franchisesByPrice`, params, { headers: {
          'Authorization': 'Bearer '+cookies.get("jwt_authorization"),
          'Content-Type': 'application/json'
        }});
        if (response.data.success) {
