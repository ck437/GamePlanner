...
function Login() {
  ...
  useEffect(() => {
    firebase.auth().onAuthStateChanged(async (user) => {
      ...
      const response = await axios.post(process.env.URL + '/api/firebase-auth-signin', param);
      if (response.data.success) {
        ...
        const userResponse = await axios.get(process.env.URL + `/api/user/${user_email}`,  { headers: {
