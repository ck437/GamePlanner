  ...
  const login = async (email1, password1) => {
    ...
    try{
      ...
      if (email1.includes("@")) {
        ...
        const response = await axios.post(process.env.URL + '/api/signin', credentials);
        if (response.data.success) {
          ...
          const userResponse = await axios.get(process.env.URL + `/api/user/${response.data.data.email}`, { headers: {
